...
$stmt->execute([$slug]);
$event = $stmt->fetch();
if (!$event) { header("HTTP/1.0 404 Not Found"); exit; }

$metaTitle = htmlspecialchars($event['seo_title'] ?: $event['name']);
$metaDesc  = htmlspecialchars($event['meta_description'] ?: substr(strip_tags($event['description']), 0, 150));
?>
<!DOCTYPE html>
<html lang="en">
<head>
